...
export default function OrganizationDashboardPage() {
  ...
  const refresh = async (organizationId = selectedOrganizationId) => {
    setLoading(true);
    const list = await organizationRepository.listMine();
    setOrganizations(list.map(({ id, displayName }) => ({ id, displayName })));
    const next = await organizationRepository.getWorkspace(organizationId || list[0]?.id);
    setWorkspace(next);
  ...
  return (
    <main className="org-dashboard-page">
      ...
      {editingEmployee && <div className="platform-modal"><form className="platform-modal-card" onSubmit={async (event: FormEvent<HTMLFormElement>) => {
        event.preventDefault(); const data = new FormData(event.currentTarget);
        try { await organizationRepository.updateEmployee(editingEmployee.id, String(data.get("position")), String(data.get("departmentId")) || null); setEditingEmployee(null); await refresh(); }
        catch (error) { setMessage(error instanceof Error ? error.message : "Error"); }
      }}><div className="modal-head"><h2>{copy.moveEmployee}</h2><button type="button" onClick={() => setEditingEmployee(null)}>×</button></div><div className="platform-form"><label><span>{copy.fullName}</span><input value={editingEmployee.name} disabled /></label><label><span>{copy.position}</span><input name="position" defaultValue={editingEmployee.position} required /></label><label><span>{copy.department}</span><select name="departmentId" defaultValue={editingEmployee.departmentId ?? ""}><option value="">—</option>{workspace.departments.map((item) => <option key={item.id} value={item.id}>{departmentPath(item, workspace.departments)}</option>)}</select></label><button className="button button-primary">{copy.save}</button></div></form></div>}
