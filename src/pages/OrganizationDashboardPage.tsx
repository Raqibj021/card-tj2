  ...
  return (
    <main className="org-dashboard-page">
      ...
      {showEmployeeForm && <div className="platform-modal"><form className="platform-modal-card" onSubmit={async (event: FormEvent<HTMLFormElement>) => {
        ...
        try { await organizationRepository.createEmployee(await employeeValues(event.currentTarget, organization.id)); setMessage(copy.active); setShowEmployeeForm(false); await refresh(); } catch (error) { setMessage(error instanceof Error ? error.message : "Error"); }
