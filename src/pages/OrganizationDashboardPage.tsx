...
export default function OrganizationDashboardPage() {
  ...
  const organizationCopy = {
    ru: { edit: "Изменить данные организации", title: "Данные организации", name: "Название организации *", type: "Тип организации *", phone: "Телефон *", email: "Электронная почта *", address: "Адрес", description: "Краткое описание", descriptionHint: "Коротко расскажите об организации", logo: "Логотип организации", auto: "Изменения сохраняются сразу. Повторная проверка администратора не требуется.", saved: "Данные организации обновлены.", save: "Сохранить изменения", imageError: "Выберите изображение размером не более 5 МБ." },
    tj: { edit: "Тағйири маълумоти ташкилот", title: "Маълумоти ташкилот", name: "Номи ташкилот *", type: "Навъи ташкилот *", phone: "Телефон *", email: "Почтаи электронӣ *", address: "Суроға", description: "Тавсифи кӯтоҳ", descriptionHint: "Дар бораи ташкилот кӯтоҳ нависед", logo: "Нишони ташкилот", auto: "Тағйирот фавран нигоҳ дошта мешавад. Санҷиши такрории маъмур лозим нест.", saved: "Маълумоти ташкилот нав шуд.", save: "Нигоҳ доштани тағйирот", imageError: "Тасвири то 5 МБ-ро интихоб кунед." },
    en: { edit: "Edit organization details", title: "Organization details", name: "Organization name *", type: "Organization type *", phone: "Phone *", email: "Email *", address: "Address", description: "Short description", descriptionHint: "Briefly describe the organization", logo: "Organization logo", auto: "Changes are saved immediately. A new administrator review is not required.", saved: "Organization details updated.", save: "Save changes", imageError: "Select an image no larger than 5 MB." }
  }[language];
    ...
    <main className="org-dashboard-page">
      <div className="site-container py-10 md:py-14">
        ...
        <div className="org-workspace-grid">
          ...
          {!selectedDepartmentId ? <section className="org-content-panel org-structure-guide"><Network size={32} /><h2>{language === "tj" ? "Рӯйхати кормандон" : language === "en" ? "Employee list" : "Список сотрудников"}</h2><p>{language === "tj" ? "Барои дидани кормандон яке аз воҳидҳои сохториро интихоб кунед." : language === "en" ? "Select a structure unit on the left to view its employees." : "Нажмите на подразделение слева, чтобы увидеть его сотрудников."}</p></section> : <section className="org-content-panel org-inline-employees"><header><div><span className="section-label">{copy.departments}</span><h2>{workspace.departments.find((item) => item.id === selectedDepartmentId)?.name}</h2><p>{language === "tj" ? "Кормандони ин воҳид ва зерсохторҳои он" : language === "en" ? "Employees in this unit and its child units" : "Сотрудники подразделения и его внутренних отделов"}</p></div><button type="button" className="button button-primary" onClick={() => setShowEmployeeForm(true)}><Plus size={17} /> {copy.add}</button></header><div className="org-inline-employees-body">{departmentEmployees.length ? <EmployeeDirectoryList employees={departmentEmployees} copy={copy} onOpen={(item) => item.cardSlug && window.open(employeePublicUrl(item.cardSlug), "_blank", "noopener,noreferrer")} onEdit={setEditingEmployee} onRemove={async (item) => { if (!window.confirm(item.name)) return; await organizationRepository.removeEmployee(item.id); await refresh(); }} /> : <div className="org-department-empty"><Users size={28} /><h3>{language === "tj" ? "Дар ин воҳид ҳоло корманд нест" : language === "en" ? "No employees in this unit yet" : "В этом подразделении пока нет сотрудников"}</h3><button className="button button-primary" onClick={() => setShowEmployeeForm(true)}><Plus size={17} /> {copy.add}</button></div>}</div></section>}
        </div>
      ...
      {showOrganizationForm && <div className="platform-modal" onMouseDown={() => setShowOrganizationForm(false)}><form className="platform-modal-card organization-profile-form" onMouseDown={(event) => event.stopPropagation()} onSubmit={async (event: FormEvent<HTMLFormElement>) => {
        event.preventDefault();
        if (savingOrganization) return;
        setSavingOrganization(true); setMessage("");
        const data = new FormData(event.currentTarget);
        try {
          let logo = organization.logo ?? "";
          const logoFile = data.get("organizationLogo");
          if (logoFile instanceof File && logoFile.size) {
            if (!logoFile.type.startsWith("image/") || logoFile.size > 5 * 1024 * 1024) throw new Error(organizationCopy.imageError);
            logo = await compressImage(logoFile);
          }
          await organizationRepository.updateProfile({
            organizationId: organization.id,
            name: String(data.get("organizationName")), type: String(data.get("organizationType")),
            phone: String(data.get("organizationPhone")), email: String(data.get("organizationEmail")),
            address: String(data.get("organizationAddress")), description: String(data.get("organizationDescription")), logo
          });
          setShowOrganizationForm(false); setMessage(organizationCopy.saved); await refresh(organization.id);
        } catch (error) { setMessage(error instanceof Error ? error.message : "Error"); }
        finally { setSavingOrganization(false); }
      }}><div className="modal-head"><div><span className="section-label">VIZORA.TJ</span><h2>{organizationCopy.title}</h2></div><button type="button" onClick={() => setShowOrganizationForm(false)}>×</button></div><div className="organization-profile-logo-row">{organization.logo ? <img className="organization-profile-logo-preview" src={organization.logo} alt="" /> : <span><Building2 size={28} /></span>}<label><strong>{organizationCopy.logo}</strong><input name="organizationLogo" type="file" accept="image/png,image/jpeg,image/webp" /><small>PNG, JPG, WEBP · 5 MB</small></label></div><div className="organization-profile-grid"><label><span>{organizationCopy.name}</span><input name="organizationName" defaultValue={organization.displayName} minLength={2} maxLength={160} required /></label><label><span>{organizationCopy.type}</span><input name="organizationType" defaultValue={organization.organizationType} minLength={2} maxLength={100} required /></label><label><span>{organizationCopy.phone}</span><input name="organizationPhone" type="tel" defaultValue={organization.phone} minLength={9} maxLength={24} required /></label><label><span>{organizationCopy.email}</span><input name="organizationEmail" type="email" defaultValue={organization.email} maxLength={160} required /></label><label className="wide"><span>{organizationCopy.address}</span><input name="organizationAddress" defaultValue={organization.address ?? ""} maxLength={240} /></label><label className="wide"><span>{organizationCopy.description}</span><textarea name="organizationDescription" defaultValue={organization.publicDescription ?? ""} maxLength={500} placeholder={organizationCopy.descriptionHint} /></label></div><div className="modal-note"><ShieldCheck size={18} /> {organizationCopy.auto}</div><button className="button button-primary button-large" disabled={savingOrganization}><Edit3 size={18} /> {organizationCopy.save}</button></form></div>}
